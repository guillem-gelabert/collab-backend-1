...
describe.only('evalVotes', () => {
  ...
  const voteController = new VoteController(operationController);

  it('calls executeOperation if all votes are affirmative', () => {
    const spy = jest.spyOn(operationController, 'executeOperation');
    voteController.evalVotes(mocks.oId, mocks.mockVotes.allAffirmative);
    expect(spy).toHaveBeenCalled(); 
  });

  it('doesn\'t executeOperation if all votes are affirmative', () => {
    const spy = jest.spyOn(operationController, 'rejectOperation');
    voteController.evalVotes(mocks.oId, mocks.mockVotes.allNegative);
    expect(spy).toHaveBeenCalled(); 
  });

  it('calls executeOperation if votes are mixed', () => {
    const spy = jest.spyOn(operationController, 'rejectOperation');
    voteController.evalVotes(mocks.oId, mocks.mockVotes.mixed);
    expect(spy).toHaveBeenCalled(); 
  });
});
